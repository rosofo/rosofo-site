...
void main() {
    ...
    vec4 lineVec = vec4(vec2(u_p1) - vec2(p2), 0.0, 0.0);
    vec4 rotAroundP1 = vec4(qtTurn * vec2(u_p1 - sc), 0.0, 0.0);
    vec4 line = (normalize(lineVec) * normalize(rotAroundP1)).z == 1.0 ? vec4(1.0) : vec4(0.0);
